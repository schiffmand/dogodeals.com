...
//connect to database with godaddy vars and settings.

$hostname='dd_DEALS.db.4091862.hostedresource.com';
$username='dd_DEALS';
$password='dd_pasS1';
$dbname='dd_DEALS';
...
//$query = sprintf("SELECT R.rest_name,R.rest_address, E.event_day_of_week, E.event_type, E.event_start_time, E.event_end_time, E.event_category, ( 3959 * ACOS( COS( RADIANS($latitude) ) * COS( RADIANS( R.rest_latitude ) ) * COS( RADIANS( R.rest_longitude ) - RADIANS($longitude) ) + SIN( RADIANS($latitude) ) * SIN( RADIANS( R.rest_latitude ) ) ) ) AS Distance FROM EVENTS e, RESTAURANT_2 R WHERE 1=1 AND R.rest_id = E.rest_id HAVING Distance <= $miles ORDER BY Distance LIMIT 0, 20");

$query = sprintf("SELECT R.rest_name,R.rest_address, E.event_day_of_week, E.event_type, E.event_start_time, E.event_end_time, E.event_category, R.rest_latitude, R.rest_longitude, ( 3959 * ACOS( COS( RADIANS('%s') ) * COS( RADIANS( R.rest_latitude ) ) * COS( RADIANS( R.rest_longitude ) - RADIANS('%s') ) + SIN( RADIANS('%s') ) * SIN( RADIANS( R.rest_latitude ) ) ) ) AS Distance FROM EVENTS e, RESTAURANT_2 R WHERE 1=1 AND R.rest_id = E.rest_id AND E.event_day_of_week LIKE ('%s') HAVING Distance <= '%s' ORDER BY Distance LIMIT 0, 20",
...
//	$newnode->setAttribute("Restaurant Category",$row['rest_category']);
//	$newnode->setAttribute("Restaurant Phone",$row['rest_phone']);
